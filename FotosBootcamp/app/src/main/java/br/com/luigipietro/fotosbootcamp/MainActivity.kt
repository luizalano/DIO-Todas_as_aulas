package br.com.luigipietro.fotosbootcamp
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        pick_button.setOnClickListener {
            /*
             VERSION_CODES_M é a versão Marshmallow, que foi a escolhida na criação do projeto
             */
        ...
        when(requestCode) {
            PERMISSION_CODE_IMAGE_PICK -> {
                if (grantResults.size > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(this, "Não posso pegar as fotos!", Toast.LENGTH_LONG).show()
                }
            }

            PERMISSION_CODE_CAMERA_CAPTURE -> {
                /*
                Tem que ter as permissões de acessar a câmera E gravar a foto tirada
                 */
                if (grantResults.size > 1
                    ...
                    Toast.makeText(this, "Não posso abrir a câmera!", Toast.LENGTH_LONG).show()
                }
            }
        }
    }
